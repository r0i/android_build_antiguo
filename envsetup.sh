...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^next_") ; then
       NEXT_BUILD=$(echo -n $1 | sed -e 's/^next_//g')
       export BUILD_NUMBER=$((date +%s%N ; echo $NEXT_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       NEXT_BUILD=
    fi
    export NEXT_BUILD
...
function breakfast()
{
    target=$1
    local variant=$2
    NEXT_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/next/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the NEXT model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
            lunch next_$target-$variant
        fi
    fi
    return $?
}
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the Next-OS github
        T=$(gettop)
        pushd $T > /dev/null
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var NEXT_VERSION)
    ...
    if (adb shell getprop ro.next.device | grep -q "$NEXT_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $NEXT_BUILD, run away!"
    fi
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $NEXT_FIXUP_COMMON_OUT ]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.next.device | grep -q "$NEXT_BUILD") || [ "$FORCE_PUSH" == "true" ];
    then
...
# Execute the contents of any vendorsetup.sh files we can find (exluding generic).
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/next/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
